...
zeroyt7.on("group-participants-update", async (anu) => {
    ...
    try {
      ...
      if (anu.action == "add" && mem.includes(zeroyt7.user.jid)) {
        zeroyt7.sendMessage(anu.jid, "Halo!.. saya ItsukiBot saya akan membatu mempermudah kehidupan..seperti membuat sticker dan lain-lain. untuk meulai silahkan ketik !menu.", "conversation")
      }
      if (!isWelkom) return
      if (anu.action == "add" && !mem.includes(zeroyt7.user.jid)) {
        ...
        wel = `Halo @${anu_user} \nWelcome In ${mdata.subject} \nJangan Lupa Donasi Ya kak\n• Wajib Intro dulu ngab\n• Nama : \n• Askot :\n• Umur :\nMoga betah yak, btw jangan jadi kutu loncat`
        ...
        sendButImage(mdata.id, wel, "©Created : Radit Ganz", buff, but)
      }
      if (!isWelkom) return
      if (anu.action == "remove" && !mem.includes(zeroyt7.user.jid)) {
        ...
        sendButImage(mdata.id, out, "©Created : Radit Ganz", buff, but)
      }
